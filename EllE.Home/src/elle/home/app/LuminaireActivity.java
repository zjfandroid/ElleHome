...
import elle.home.app.view.NumberSeekBar;
...
public class LuminaireActivity extends BaseActivity {
	...
	OnRecvListener recvListener = new OnRecvListener(){
		...
		public void OnRecvData(PacketCheck packetcheck) {
			if(packetcheck!=null){
				...
				colorLight = DataExchange.twoByteToInt(packetcheck.xdata[12], packetcheck.xdata[13]);
				if(colorLight<0||colorLight>255){
					colorLight = 128;
				}
				...
				if(DataExchange.twoByteToInt(packetcheck.xdata[8], packetcheck.xdata[9])>0){
					...
					colorRed = DataExchange.twoByteToInt(packetcheck.xdata[2], packetcheck.xdata[3]);
					colorGreen = DataExchange.twoByteToInt(packetcheck.xdata[4], packetcheck.xdata[5]);
//					colorBlue = DataExchange.twoByteToInt(packetcheck.xdata[6], packetcheck.xdata[7]);
					colorLight = DataExchange.twoByteToInt(packetcheck.xdata[12], packetcheck.xdata[13]);
	...
	public int luxToColor(int a){
		int tmp = 0;
		tmp = (int)((float)a*(float)300/(float)(colorLight));
		return tmp;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setContentView(R.layout.activity_luminaire);
		...
		randombtn = (Button)this.findViewById(R.id.btnRandom);
	...
	private void setColorLight(int time) {
		...
		ShowInfo.printLogW(colorRed + "----" + colorGreen + "_________color________" + colorBlue + "________" + colorLight);
		
		packet.lightColor(DataExchange.longToEightByte(dev.mac), recvListener, colorRed, colorGreen, colorBlue, 0, colorLight, 500);
	...
	public int colorToLux(int a){
		int tmp = 0;
		tmp = (int)((float)a*(float)colorLight/(float)300);
		return tmp;
	}
	
	public void doClickOn(View v){
		if(rgbLightStatus==true){
			
			LuminaireControlPacket packetoff = new LuminaireControlPacket(conip,conport);
			if(connectStatus == PublicDefine.ConnectRemote){
			...
			v.setBackgroundResource(R.drawable.selecter_light_off);
		}else{
			
			LuminaireControlPacket packeton = new LuminaireControlPacket(conip,conport);
			if(connectStatus == PublicDefine.ConnectRemote){
			...
			v.setBackgroundResource(R.drawable.selecter_light_on);
		}
	}
...
//		btnShake.setOnOff(dev.getCanShake(mContext));
		...
		randombtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				setRandomBtnBackground();
				packet.setImportance(BasicPacket.ImportHigh);
				packet.lightFree(DataExchange.longToEightByte(dev.mac), recvListener, randomStatus);
				autoBinder.addPacketToSend(packet);
			}
		});
		...
		NumberSeekBar seek = (NumberSeekBar) findViewById(R.id.seek_cold);
		seek.setOnSeekBarChangeListener(mOnSeekBarChangeListener);
		seek.setProgress(colorRed);
		seek.setText("冷白");
		
		seek = ((NumberSeekBar) findViewById(R.id.seek_warm));
		seek.setOnSeekBarChangeListener(mOnSeekBarChangeListener);
		seek.setProgress(colorGreen);
		seek.setText("暖白");
		
		seek = ((NumberSeekBar) findViewById(R.id.seek_lux));
		seek.setOnSeekBarChangeListener(mOnSeekBarChangeListener);
		seek.setProgress(colorLight);
		seek.setText("亮度");
	}
	...
	private void setRandomBtnBackground() {
		if(randomStatus){
			randombtn.setBackgroundResource(R.drawable.selecter_light_random);
		}else{
			randombtn.setBackgroundResource(R.drawable.selecter_light_random_off);
		}
	}
